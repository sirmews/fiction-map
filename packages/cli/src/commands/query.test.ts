...
describe("query commands", () => {
  ...
  it("explains a node", async () => {
    ...
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes("Node main-hall (scene)"))).toBe(true)
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes("Incoming: enter-hall"))).toBe(true)
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes("Outgoing: descend"))).toBe(true)
  })
  ...
  it("exits for unknown ids", async () => {
    ...
    await expect(explain("missing", { rootDir: TEST_DIR, outputDir })).rejects.toThrow("process.exit(1)")
  ...
  it("lists known graph ids when a graph lookup fails", async () => {
    ...
    await expect(showGraph("missing-graph", { rootDir: TEST_DIR, outputDir })).rejects.toThrow("process.exit(1)")
    expect(exitSpy).toHaveBeenCalledWith(1)
    expect(errorSpy.mock.calls.some((call) => String(call[0]).includes('Known graph ids: "library".'))).toBe(true)
  })
})
