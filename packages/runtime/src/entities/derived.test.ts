...
import { activateEntity, createInitialState, grantEntity, unlockEntity } from "../core/state"
...
describe("deriveEntityState", () => {
  ...
  it("reports owned, active, unlocked, and effective entity ids", () => {
    ...
    expect(derived.effectiveEntityIds).toEqual(new Set(["sword", "dexterity", "cave"]))
  })
      ...
      entities: [
        {
          id: "strong",
          type: "trait",
          modifiers: [{ target: "stats.strength", operation: "add", value: 5 }],
        },
      ],
    })
    ...
    const world = defineWorld(registry, {
      ...
      entities: [
        { id: "key", type: "item" },
        {
          id: "locked-door",
          type: "location",
          prerequisites: [{ kind: "entity", target: "key", operator: "has" }],
        },
      ],
    })
